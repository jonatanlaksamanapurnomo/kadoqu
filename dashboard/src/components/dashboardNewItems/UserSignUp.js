...
const QUERY_GET_TOTAL_SIGNUP = gql`
  query getUserSignUp($startDate: ParsedDateInput, $endDate: ParsedDateInput) {
    getUserSignUp(startDate: $startDate, endDate: $endDate)
  }
`;
    ...
    return (
      ...
      >
        {({ loading, error, data }) => {
          ...
          const products = data.getUserSignUp;
          const chartData = {
            labels: Array.from(
              {
                length:
