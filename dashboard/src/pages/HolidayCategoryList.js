...
class HolidayCategoryList extends React.Component {
  state = {
    isModalOpen: false
  };
  render() {
              ...
              onClick={() => {
                Swal.fire({
                  ...
                  if (result.value) {
                    this.props.client
                      .mutate({
                        mutation: MUTATION_DELETE_HOLIDAY_CATEGORY,
                        variables: {
                          id: row.id
                        },
                        refetchQueries: [
                          {
                            query: QUERY_GET_HOLIDAY_CATEGORIES
                          }
                        ]
                      })
                      .then(() =>
    ...
    return (
      <React.Fragment>
          ...
          {({ loading, error, data }) =>
            loading ? (
              ...
              <ToolkitProvider
                bootstrap4
                keyField="id"
                data={data.getHolidayCategories}
                columns={columns}
                search={{
              ...
              >
                {props => (
                  <Card>
                    <CardHeader>
                      <h2 className="mb-0">Holiday Categories</h2>
                    </CardHeader>
                    <CardBody>
                      ...
                      <BootstrapTable
                        ...
                        cellEdit={cellEditFactory({
                          ...
                          beforeSaveCell: (
                            ...
                            this.props.client
                              .mutate({
                                mutation: MUTATION_UPDATE_HOLIDAY_CATEGORY,
                                variables: {
                                  id: row.id,
                                  newName: newValue
                                },
        ...
        <AddStoreCategoryModal
          ...
          refetchQueries={[{ query: QUERY_GET_HOLIDAY_CATEGORIES }]}
        />
      </React.Fragment>
    );
  }
...
export default withApollo(HolidayCategoryList);
