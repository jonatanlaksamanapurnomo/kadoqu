...
class IWD_Opc_Block_Onepage_Wrapper extends Mage_Checkout_Block_Onepage_Abstract
{
    ...
    public function getJsonConfig()
    {
        ...
        $config['isIwdGiftCardEnabled'] = $opcHelper->isIwdGiftCardEnable();
        $config['giftCardApplyUrl'] = $this->getUrl(
            $opcFrontName . '/json/applyGiftCard',
            array('_secure' => $secure)
        );
        $config['giftCardRemoveUrl'] = $this->getUrl(
            $opcFrontName . '/json/removeGiftCard',
            array('_secure' => $secure)
        );

        if (Mage::getConfig()->getModuleConfig('OPG_Square')->is('active', 'true')) {
            $config['opcSquareAppId'] = Mage::helper('square')->getAppId(Mage::helper('square')->getMode());
        }
        ...
        $config['isStoreCreditEnabled'] = (int)$opcHelper->isStoreCreditEnable() && $this->isCustomerLoggedIn();
        $config['storeCreditApplyUrl'] = $this->getUrl(
            $opcFrontName . '/json/applyStoreCredit',
            array('_secure' => $secure)
        );
        $config['storeCreditRemoveUrl'] = $this->getUrl(
            $opcFrontName . '/json/removeStoreCredit',
            array('_secure' => $secure)
        );
