...
class IWD_Opc_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function isIwdGiftCardEnable()
    {
        $status = Mage::getConfig()->getModuleConfig('IWD_GiftCard')->is('active', 'true')
            && Mage::getConfig()->getModuleConfig('IWD_GiftCardAccount')->is('active', 'true');

        if ($status) {
            $status = Mage::helper('iwd_giftcard')->isEnable()
                && Mage::helper('iwd_giftcardaccount')->isEnable();
        }
    ...
    {
        $status = Mage::getConfig()->getModuleConfig('IWD_StoreCredit')->is('active', 'true');
        if ($status) {
            $status = Mage::helper('iwd_storecredit')->isEnable();
        }
    ...
    {
        $status = Mage::getConfig()->getModuleConfig('IWD_RecurringPayments')->is('active', 'true');
        if ($status) {
            $status = Mage::helper('iwd_recurringpayments')->isEnable();
        }
