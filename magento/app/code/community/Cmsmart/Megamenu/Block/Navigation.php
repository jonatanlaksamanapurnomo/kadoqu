...
class Cmsmart_Megamenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
   ...
   protected function _renderCategoryMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false,
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if(@$megamenu[0]['active_static_block']==1) {
			$cl_parent = 'parent'; 
		} else {
			$cl_parent = ''; 
		}
        ...
        foreach ($attributes as $attrName => $attrValue) {                       
            $htmlLi .= ' ' . $attrName .  '="' . str_replace('"', '\"', $attrValue) . ' ' . $nothumbnail . ' ' . $cl_parent . '"';
                  } 
        ...
        if ($childrenWrapClass) {
            ...
            if($level == 0) {
                $width_menu = '100%';
                $html[] = '<div class="level' . $level .' '. $addclass .' column'.$this->getLevle($id).'" style="width:'.$width_menu. '">';   
            } else {
                  ...
                  if(@$megamenu[0]['category_children']==0)   {   
                    ...
                    if($htmlChildren) {
                        if($level == 0){
                            ...
                            $html[] = '<ul class="catagory_children" style="width:'.$widthchildren.'px"><li class="li-wrapper">'.$htmlChildren.'</li></ul>'; 
    ...
    public function widthp($idc){
        ...
        $cent = ($column/$widthchildrenli)*100;
            ...
            $reviews = Mage::getModel('review/review')
                ...
                if($number < 3 ) { if(($i%2)==0){
                    ...
                    if(($i%3)==0){
                     ...
                     $html .= '<a class="levelproduct-img" href="'. $_product->getProductUrl(). '" title="' . $this->htmlEscape($_product->getName()). '"> 
                ...
                $html .= '<a  class="product-name" href="'. $_product->getProductUrl(). '" title="' . $this->htmlEscape($_product->getName()). '">'.$_product->getName().'</a>';
                if($level == 0){
                    if (count($reviews) > 0) {
                ...
                if($level==0){
                $html .= '<a class="btn-cart btn-featurecart" href="'.$this->helper('checkout/cart')->getAddUrl($_product).'"><span><span>Add to Cart</span></span></a>';
    ...
    public function getLabel($megamenu){
         if(@$megamenu[0]['active_label'] == 1){
			 $label_name = strip_tags($megamenu[0]['label']);
             return '<span  class="cat-label pin-bottom '.$label_name.'">'.$label_name.'</span>';
         } else {
            return ;
         } 
    }
