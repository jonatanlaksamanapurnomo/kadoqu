    ...
    public function savegiftboxAction()
    {
        ...
        if ($wraptype) {
            ...
            if ($wraptype == 1) {
                ...
                Mage::getSingleton('checkout/session')->addSuccess("Gift wrap was saved successfully.");
            }
            if ($wraptype == 2) {
                ...
                foreach ($wrapall as $itemId) {
                    ...
                    if (floatval($data['qty_wrap_' . $itemId]) > floatval(Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id']))) {
                        Mage::getSingleton('checkout/session')->addError('There are too many items in one gift box. The maximum number allowed is (lấy giá trị config trong backend). Automatically added new gift box(es) to cart.');
