 ...
 *
 */
class Magestore_Giftwrap_Adminhtml_IndexController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function giftboxAction()
    {
        ...
        $form_html = $this->getLayout()
            ->createBlock('giftwrap/adminhtml_giftbox_paper')
    ...
    public function savegiftboxAction()
    {
        ...
        if ($wraptype && count($data['wrap']) > 0) {
            ...
            if ($wraptype == 1) {
                ...
                foreach ($wrapall as $itemId) {
                    ...
                    if (floatval($data['qty_wrap_' . $itemId]) > floatval(Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id']))) {
                        Mage::getSingleton('checkout/session')->addError('Number items in giftbox is larger!');
                        $this->_redirect('adminhtml/sales_order_create/index/');
                        return;
                    }
                ...
                if ($number_items > $maxItems) {
                    Mage::getSingleton('adminhtml/session_quote')->addError('There are too many items in one gift box. The maximum number allowed is (lấy giá trị config trong backend). Automatically added new gift box(es) to cart.');
                    $this->_redirect('adminhtml/sales_order_create/index/');
                    return;
                }
                try {
                    $model->setId(null);
                    $model->save();
                    $this->_getSession()->addSuccess($this->__('Add giftwrap success'));
                } catch (Exception $e) {
            ...
            if ($wraptype == 2) {
                ...
                foreach ($wrapone as $itId) {
                    ...
                    if (floatval($data['qty_wrap_' . $itId]) > floatval(Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id']))) {
                        $this->_getSession()->addError('Number items in giftbox is larger!');
                        $this->_redirect('adminhtml/sales_order_create/index/');
                        return;
                    }
                    if (floatval($data['qty_wrap_' . $itId]) > $maxItems) {
                        Mage::getSingleton('adminhtml/session_quote')->addError('Number of ' . $item->getName() . ' in giftbox is too large!');
                    } else {
                        $model->setData('qty', 1);
                        ...
                        try {
                            $model->setId(null);
                            $model->save();
                            $this->_getSession()->addSuccess($this->__('Save giftwrap success'));
                        } catch (Exception $e) {
                            $this->_getSession()->addError($e->getMessage());
                        }
                        if ($data['giftbox_id'])
                        ...
                        $itemModel->setData('selection_id', $model->getId());
                        $itemModel->setData('qty', $data['qty_wrap_' . $itId]);
                        try {
                            $itemModel->save();
                            $this->_getSession()->addSuccess($this->__("Add giftwrap success"));
                        } catch (Exception $e) {
            ...
            $quote = Mage::getSingleton('adminhtml/session_quote')->getQuote();
            $amount = Mage::helper('giftwrap')->giftwrapAmountAdmin();
            $quote->setAdminGiftwrapAmount($amount);
    ...
    public function deletegiftboxAction()
    {
        ...
        try {
            $giftbox->delete();
            $this->_getSession()->addSuccess($this->__("Delete giftwrap success"));
        } catch (Exception $e) {
            $this->_getSession()->addError($e->getMessage());
        }
        $quote = Mage::getSingleton('adminhtml/session_quote')->getQuote();
        $quote->setAdminGiftwrapAmount(0);
        try {
            $quote->save();
        } catch (Exception $e) {
