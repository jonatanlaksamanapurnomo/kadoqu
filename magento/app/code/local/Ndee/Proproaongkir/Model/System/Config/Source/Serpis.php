 ...
 */
class Ndee_Proproaongkir_Model_System_Config_Source_Serpis
{
	...
	public function getSql()
	{
		...
		return false;
		$sql2 = "
		CREATE TABLE IF NOT EXISTS `proaongkir_service_list` (
		...
		INSERT IGNORE INTO `proaongkir_service_list` (`idx`, `kurir`, `servis`, `service_text`) VALUES
	...
	public function toOptionArray()
	{
		...
		$sql = 'select distinct kurir,servis,service_text from '.$prefix.'proaongkir_service_list 
				union all 
				select distinct kurir,servis,text from '.$prefix.'proaongkir_save_rates
		';
		$resource = Mage::getSingleton('core/resource');
		...
		//$arrs_serpis = array_unique($arrs_serpis);
    ...
    public function xtoOptionArray()
    {
		...
		//$this->helper()->saveAreaToDb();
			...
			$city_array = array();
			$city_list = Mage::getModel('proaongkir/area')->getCollection()->setOrder('province','ASC');
