...
class Amasty_Rules_Model_Rule_Abstract
{
    ...
    public function prepareDiscount($discount, $address, $rule)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($rule->getPriceSelector() == Amasty_Rules_Model_Observer::BASED_ON_BIGGEST) {
                if (array_key_exists($item->getId(), $discount)) {
    ...
    {
        if ($item->getPrice() != $item->getProduct()->getPrice() && isset($discount[$item->getId()]['percent'])) {
            $givenPercent = $discount[$item->getId()]['percent'];
            $currentPercent = (1 - $item->getPrice() / $item->getProduct()->getPrice()) * 100;
            $recalculatePercent = $givenPercent - $currentPercent;
            $currentDiscount = $item->getProduct()->getPrice() - $item->getPrice();

            if ($recalculatePercent < 0) {
                $discount[$item->getId()] = [];
            } elseif($recalculatePercent > 0) {
                $q = $currentPercent / $recalculatePercent;
                $addDiscount = $currentDiscount / $q;
                $discount[$item->getId()]['discount'] = $addDiscount;
                $discount[$item->getId()]['base_discount'] = $addDiscount;
