...
awk "{sub(/edge:.+/,\"edge:$(git rev-parse --verify HEAD)\")}1" deploy.yml >dep.yml
...
curl 'https://edge-stag.kadoqu.com/' -H 'content-type: application/json' --data-binary '{"operationName":null,"variables":{},"query":"mutation {  addDeployHistory(name: \"'"$USERNAME"'" , status: \"'"$COMMITID"'" , product: \"edge\")}"}' --compressed
